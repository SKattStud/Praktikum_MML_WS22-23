...
from config import MAXLEN, SPAM, NON_SPAM, MODEL_DIR
...
class Trainer:
    ...
    def execute(self):
        ...
        model = Sequential()
        model.add(layers.Embedding(len(tokenizer.word_index) + 1, 20, input_length=MAXLEN))
        model.add(layers.Conv1D(128, 5, activation='relu'))
        model.add(layers.GlobalMaxPool1D())
        model.add(layers.Dense(20, activation='relu'))
        model.add(layers.Dense(1, activation='sigmoid'))
        model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
        model.summary()  # debug command
        ...
        model.fit(email_liste_train, label_liste_train, batch_size=16, epochs=40, validation_data=[email_liste_test, label_liste_test])
